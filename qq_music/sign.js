...
try {
  ...
  const music_uin = process.env.MUSIC_ID;
  data = `{\"req1\":{\"module\":\"QQConnectLogin.LoginServer\",\"method\":\"QQLogin\",\"param\":{\"musicid\":${music_uin},\"musickey\":\"${music_key}\"}}}`
  const result = getSign(data);
  // GitHub Actions output
  const outputFile = process.env.GITHUB_OUTPUT;

  if (outputFile) {
    fs.appendFileSync(outputFile, `sign=${result}\n`);
  } else {
    console.log(result);  // fallback for local use
  }
} catch (e) {
  console.error("Invalid JSON string passed:", e);
}
